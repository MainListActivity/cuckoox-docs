...
import {type CollectionEntry, type CollectionKey, getCollection, getEntry, type SchemaContext,} from 'astro:content';
...
export async function generateBreadcrumb(
    name: CollectionKey,
    slug: string
): Promise<{ items: { title: string; href: string }[], home?: string }> {
    ...
    return {items, home: `${import.meta.env.BASE_URL}/${urlForCollection[name]}`};
}
